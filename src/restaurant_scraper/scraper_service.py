...
class JustEatScraper:
    ...
    def scrape_all_restaurants(self):
        cur.execute("SELECT * FROM menu_slugs;")
        for slug in cur:
            time.sleep(8)
            self.scrape_restaurant(f"{JUSTEAT_URL + slug[1]}/menu")
    ...
    def scrape_restaurant(self, url):
        ...
        post_req = requests.post(
            "http://localhost:2999/api/restaurants/create", json=self.payload
        )
        print(
            f"Status Code:{post_req.status_code} Scraped data for url ({url}) sent to database"
        )
    ...
    def _scrape_restaurant_data(self, soup):
        ...
        self.payload["rating"] = self._format_rating(rating)

    def _format_rating(self, rating):
        arr = rating.split()
        rating = int(float(arr[0])*2)
        max_rating = int(float(arr[-1])*2)
